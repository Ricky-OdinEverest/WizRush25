...
void UBTTask_RotateToFaceTarget::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	...
	else
	{
		...
		FRotator TargetYaw(0.f, TargetRot.Yaw, 0.f);

		Memory->OwningPawn->SetActorRotation(TargetYaw);
	}
}
...
//	const float AngleDiff = UKismetMathLibrary::DegAcos(DotResult);
	...
	// Forward direction flattened to X–Y
	FVector OwnerForward2D = QueryPawn->GetActorForwardVector();
	OwnerForward2D.Z = 0.f;
	OwnerForward2D.Normalize();

	// Direction to target flattened to X–Y
	FVector ToTarget2D = TargetActor->GetActorLocation() - QueryPawn->GetActorLocation();
	ToTarget2D.Z = 0.f;
	ToTarget2D.Normalize();

	// Dot product in 2D plane
	const float DotResult = FVector::DotProduct(OwnerForward2D, ToTarget2D);
	const float AngleDiff = FMath::RadiansToDegrees(FMath::Acos(FMath::Clamp(DotResult, -1.f, 1.f)));
